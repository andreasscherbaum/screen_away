...
# but so we have to execute the screen executable once
# disable locale
...
# workaround for not running in a screen but controlled by an external file
$screen_away_used = 1;
...
$socket = $ENV{'HOME'} . '/.irssi/external_screen_status';
